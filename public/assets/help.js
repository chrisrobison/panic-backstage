...
const HELP_CONTENT = {
  ...
  'recurring-events': `
    ...
    <h3>Conflicts</h3>
    <p>Every generated date is checked against existing bookings at that venue, exactly like a normal event create. When you use the <strong>Recurrence</strong> panel on an existing event, a conflicted date no longer blocks the whole series — a <strong>Resolve recurring-event conflicts</strong> dialog lists every date that collides with an existing booking (which event it collides with, and its status), and for each one you can either move it to a different room that's free that day or skip that single date. The rest of the pattern is created as planned either way; skipped dates are simply left out of the series.</p>
    <p class="help-tip">💡 Starting a series from the event wizard, or asking the <a href="#help-ai-assistant">AI Assistant</a> to set one up, does not go through this dialog — a conflict on any date there still rejects the whole request with an error naming the colliding date(s), so you can adjust the pattern and try again. Use the Recurrence panel directly if you want per-date skip/reassign control.</p>
