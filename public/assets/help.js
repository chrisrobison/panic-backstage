...
const HELP_CONTENT = {
  welcome: `
    ...
    <p>Panic Backstage helps a venue run a show from the first inquiry through final settlement. It keeps the booking conversation, contract, lineup, run sheet, flyers, ticketing notes, open items, door list, and money in one place so a small team can hand off cleanly between bookers, promoters, designers, and night-of-show staff.</p>
  ...
  overview: `
    ...
    <p class="help-tip">💡 If a teammate edits this same event elsewhere, an open Details tab picks up the change automatically — the fields that actually changed briefly flash so you notice, without reloading or losing your place. If you're actively mid-edit when it arrives, Backstage never overwrites what you typed — it holds the update and applies it (with the same flash) the moment you move on to another field. A small dot in the top bar shows whether live updates are currently connected.</p>
  ...
  'recurring-events': `
    ...
    <h3>Conflicts</h3>
    <p>Every generated date is checked against existing bookings at that venue, exactly like a normal event create. A conflicted date no longer blocks the whole series — a <strong>Resolve recurring-event conflicts</strong> dialog lists every date that collides with an existing booking (which event it collides with, and its status), and for each one you can either move it to a different room that's free that day or skip that single date. The rest of the pattern is created as planned either way; skipped dates are simply left out of the series. This dialog is the same one no matter which of the three ways below you start or apply a series from — cancelling out of it (Escape, the backdrop, or its Cancel button) just leaves the series uncreated without undoing anything else that already happened (the event itself, if the wizard created one; or the AI proposal, which stays pending so you can try Apply again later).</p>
    ...
    <h3>Cancelling a series</h3>
    <p>Use <strong>Cancel series</strong> in the Recurrence panel to cancel every not-yet-happened occurrence in one step — today's occurrence included, but nothing in the past. Each one is simply set to <em>Cancelled</em>, exactly like cancelling a single event by hand, which immediately frees its date and room for rebooking. Occurrences that already happened, or are already Cancelled/Completed/Settled, are left alone. This only changes status — it doesn't remove anything from the series or delete any occurrence, so the full history stays visible in the panel.</p>
    ...
    <p class="help-tip">💡 Aside from <strong>Cancel series</strong>, there's no other bulk edit across a series (no "change this and all future occurrences"). Each occurrence is a completely normal event once created — update its time, venue, staffing, or anything else the same way you would for any other show.</p>
    ...
    <ul>
      ...
      <li><strong>Propose a change</strong> — two specific kinds, each ending in a proposal card in the chat with <strong>Apply</strong> / <strong>Discard</strong> buttons, never an immediate change:
        <ul>
          ...
          <li><em>Recurring series</em> — e.g. "turn this into a weekly series for the next 8 weeks." Subject to the exact same rules as building one by hand in <a href="#help-recurring-events">Recurring events</a>: max 52 occurrences, none more than 90 days out. A room conflict on some of the dates doesn't stop the proposal — the assistant tells you which dates conflict, and you resolve them (skip, or move to a free room) the same way as anywhere else, when you click Apply.</li>
        </ul>
      </li>
    </ul>
