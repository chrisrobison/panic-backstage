        ...
        <table class="data-table admin-table">
          ...
          <tbody>
            ${rooms.map((r) => `<tr class="${Number(r.active) ? '' : 'muted-row'}">
              <td><strong>${esc(r.name)}</strong>${r.contract_name ? ` <span class="muted small">(contracts: ${esc(r.contract_name)})</span>` : ''}${r.description ? `<br><small class="muted">${esc(r.description)}</small>` : ''}${r.address ? `<br><small class="muted">📍 ${esc(r.address)}</small>` : ''}</td>
  ...
  openRoomModal(room = null) {
    ...
    dialog.innerHTML = `<div class="modal-card">
      ...
      <form class="grid-form padded" data-form="room">
        ...
        <label class="wide">Contract name <input name="contract_name" value="${esc(r.contract_name || '')}" placeholder="Leave blank to use the venue's name — e.g. this room does business as…"></label>
        <label class="check-label"><input type="checkbox" name="active" value="1" ${active ? 'checked' : ''}> Active</label>
