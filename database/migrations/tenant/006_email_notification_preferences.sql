-- Migration 006 (tenant): per-user email notification preferences.
-- Lets each user opt out of categories of system notification emails from the
-- Preferences page. All default to 1 (opted-in) so existing users keep their
-- current behaviour with no backfill. Transactional/security mail (magic-link
-- login, email confirmation, access-approved) is NOT governed by these flags
-- and always sends.
ALTER TABLE `users`
