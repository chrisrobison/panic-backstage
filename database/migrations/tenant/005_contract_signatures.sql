-- Migration 005 (tenant): digital signature support for contracts.
...
ALTER TABLE `contracts`
  ADD COLUMN `provider`             VARCHAR(40)  NOT NULL DEFAULT 'internal'
      COMMENT 'internal|mock|dropbox_sign|docusign'
      AFTER `status`,
  ADD COLUMN `provider_envelope_id` VARCHAR(255) DEFAULT NULL
      AFTER `provider`,
  ADD COLUMN `provider_status`      VARCHAR(80)  DEFAULT NULL
      AFTER `provider_envelope_id`,
  ADD COLUMN `preview_pdf_path`     VARCHAR(500) DEFAULT NULL
      AFTER `provider_status`,
  ADD COLUMN `final_pdf_path`       VARCHAR(500) DEFAULT NULL
      AFTER `preview_pdf_path`,
  ADD COLUMN `final_pdf_sha256`     VARCHAR(64)  DEFAULT NULL
      AFTER `final_pdf_path`,
  ADD COLUMN `fully_executed_at`    TIMESTAMP    NULL DEFAULT NULL
      AFTER `signed_at`,
