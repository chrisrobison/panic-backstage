...
CREATE TABLE `event_ledger_entries` (
  ...
  UNIQUE KEY `uq_ledger_provider_financial` (`event_id`,`source`,`source_ref_str`,`category`),
...
CREATE TABLE `event_payments` (
  ...
  `amount` decimal(10,2) NOT NULL DEFAULT 0.00,
  `processor_fee_cents` int(11) DEFAULT NULL,
  `tax_cents` int(11) DEFAULT NULL,
  `financials_status` enum('pending','partial','reported','unavailable') NOT NULL DEFAULT 'pending',
  `financials_updated_at` datetime DEFAULT NULL,
  `currency` char(3) NOT NULL DEFAULT 'USD',
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `external_ref` varchar(255) DEFAULT NULL COMMENT 'Checkout reference the active provider''s webhook echoes back for matching (Stripe: checkout session id cs_...; Square: order id). See checkout_provider.',
  `checkout_provider` varchar(20) DEFAULT NULL COMMENT 'Provider that minted the current checkout link (''stripe''|''square''), captured at link-creation time.',
  `checkout_payment_ref` varchar(255) DEFAULT NULL COMMENT 'Payment/charge reference captured from the provider webhook for refunds.',
  `checkout_url` text DEFAULT NULL COMMENT 'Cached hosted checkout URL from the last mint.',
  `checkout_expires_at` datetime DEFAULT NULL COMMENT 'Bookkeeping expiry for checkout_url reuse.',
  `receipt_token` char(64) DEFAULT NULL COMMENT 'Opaque token granting access to this payment receipt and PDF.',
  `receipt_emailed_at` datetime DEFAULT NULL COMMENT 'UTC time the receipt email was accepted by the local MTA.',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_event_payments_receipt_token` (`receipt_token`),
  KEY `idx_payments_event` (`event_id`),
  KEY `idx_payments_type` (`payment_type`),
  KEY `idx_payments_status` (`status`),
  KEY `idx_payments_checkout_ref` (`checkout_provider`,`external_ref`),
  KEY `created_by_id` (`created_by_id`),
...
CREATE TABLE `event_scanner_links` (
  ...
  `can_sell` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Opt-in: may this link ring up walk-up sales, not just scan?',
  `can_lookup` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Opt-in: may this link look up purchased tickets and admit without a QR?',
  `created_by_user_id` int(11) DEFAULT NULL,
...
CREATE TABLE `ticket_orders` (
  ...
  `payment_method` enum('cash','card','other') DEFAULT NULL COMMENT 'How the venue itself took the money (door sales); NULL for online/comp orders',
...
CREATE TABLE `tickets` (
  ...
  UNIQUE KEY `uq_tickets_event_printed_number` (`event_id`,`printed_number`),
  KEY `ticket_type_id` (`ticket_type_id`),
